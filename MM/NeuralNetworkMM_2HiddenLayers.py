...
def pack_thetas(t1, t12, t2):
    return np.concatenate((t1.reshape(-1), t12.reshape(-1), t2.reshape(-1)))
...
def unpack_thetas(thetas, input_layer_size, hidden_layer_size, num_labels):
    ...
    t12_end = hidden_layer_size * (hidden_layer_size + 1) + t1_end
    t12 = thetas[t1_end:t12_end].reshape((hidden_layer_size, hidden_layer_size + 1))
    t2 = thetas[t12_end:].reshape((num_labels, hidden_layer_size + 1))
    return t1, t12, t2

# forward propagation function
def forward_prop(X, t1, t12, t2):
    m = X.shape[0]
    ones = None
    if len(X.shape) == 1:
...
def cost_function(thetas, input_layer_size, hidden_layer_size, num_labels, X, y, reg_lambda):
    t1, t12, t2 = unpack_thetas(thetas, input_layer_size, hidden_layer_size, num_labels)
    ...
    a1, z2, a2, z3, a3, z4, h = forward_prop(X, t1, t12, t2)
    costPositive = -Y * np.log(h).T
    costNegative = (1 - Y) * np.log(1 - h).T
    cost = costPositive - costNegative
    J = np.sum(cost) / m
    
    t1n, t12n, t2n = t1, t12, t2
    t1n[:,0], t12n[:,0], t2n[:,0] = 0,0,0    
    st2 = sum(sum(t1n**2)) + sum(sum(t12n**2)) + sum(sum(t2n**2))
...
def cost_function_prime(thetas, input_layer_size, hidden_layer_size, num_labels, X, y, reg_lambda):
    ...
    for i, row in enumerate(X):
        a1, z2, a2, z3, a3, z4, a4 = forward_prop(row, t1, t12, t2)
        
        # Backprop
        d4 = a4 - Y[i, :].T
        d3 = np.dot(t2f.T, d4) * sigmoid_prime(z3)        
        d2 = np.dot(t12f.T, d3) * sigmoid_prime(z2)
    ...
    Theta3_grad = (1.0 / m) * Delta3
    if reg_lambda != 0:
        Theta1_grad[:, 1:] = Theta1_grad[:, 1:] + (reg_lambda / m) * t1f
        Theta2_grad[:, 1:] = Theta2_grad[:, 1:] + (reg_lambda / m) * t12f
        Theta3_grad[:, 1:] = Theta3_grad[:, 1:] + (reg_lambda / m) * t2f
    
    return pack_thetas(Theta1_grad, Theta2_grad, Theta3_grad)
...
def fit(X, y, maxiter):
    ...
    theta12_0 = rand_init(hidden_layer_size, hidden_layer_size, epsilon_init)
    theta2_0 = rand_init(hidden_layer_size, num_labels, epsilon_init)
    thetas0 = pack_thetas(theta1_0, theta12_0, theta2_0)
    
    options = {'maxiter': maxiter}
    res = optimize.minimize(cost_function, thetas0, jac=cost_function_prime, method=method, 
                             args=(input_layer_size, hidden_layer_size, num_labels, X, y, reg_lambda), options=options)
    
    t1, t12, t2 = unpack_thetas(res.x, input_layer_size, hidden_layer_size, num_labels)
    return t1, t12, t2
...
def checkNNgradients(reg_lambda):
    ...
    Theta12 = debugInitializeWeights(hidden_layer_size, hidden_layer_size);
    Theta2 = debugInitializeWeights(num_labels, hidden_layer_size);
    X  = debugInitializeWeights(m, input_layer_size - 1);
    y  = 1 + np.mod(range(m), num_labels).T
    nn_params = pack_thetas(Theta1, Theta12, Theta2)
...
def predict(X, theta1, theta12, theta2):
    a1, z2, a2, z3, a3, z4, a4 = forward_prop(X, theta1, theta12, theta2)
    return a4
...
hidden_layer_size = 20 # nodes in hidden layer
method = 'TNC'         # minimisation method
maxiter = 500          # max iterations of minisation function
...
initial_Theta12 = rand_init(hidden_layer_size, hidden_layer_size, epsilon_init)
initial_Theta2 = rand_init(hidden_layer_size, num_labels, epsilon_init)
initial_nn_params = pack_thetas(initial_Theta1, initial_Theta12, initial_Theta2)
...
Theta1Opt, Theta12Opt, Theta2Opt = fit(X_train, y_train, maxiter)

# predict on validation dataset
prediction = np.argmax(predict(X_val, Theta1Opt, Theta12Opt, Theta2Opt), axis = 0)
acc1 = accuracy(prediction, y_val)
print('accuracy',acc1)
# gradients are good but not very accurate!
